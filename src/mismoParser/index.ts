...
export async function parseXmlToJson(xmlString: string): Promise<z.infer<typeof LoanApplicationSchema>> {
    try {
        ...
        // Extract data into a structured JS object
        //console.dir(result,{depth:null})
        const summaryData = extractLoanSummaryData(result);
        return summaryData;
        
    } catch (error) {
        console.error((error as Error).stack); // Log stack trace for debugging
        throw error;
    }
}
